...
function outputFormat(value) {
    if (!isOutputFormat(value)) {
        throw new InvalidArgumentError("应为 terminal、json、html 或 sarif。");
    }
    return value;
}
function failThreshold(value) {
    if (!isFailThreshold(value)) {
        throw new InvalidArgumentError("应为 critical、high、medium、low、info 或 never。");
    }
    return value;
}
...
function configureChineseHelp(command) {
    return command
        ...
        outputError: (message, write) => {
            const localized = message
                ...
                .replace(/option '([^']+)' argument '([^']+)' is invalid\.\s*/g, "选项“$1”的参数“$2”无效。")
                .replace(/option '([^']+)' argument missing/g, "选项“$1”缺少参数")
                .replace(/missing required argument '([^']+)'/g, "缺少必需参数“$1”")
...
configureChineseHelp(program)
    ...
    .description("面向 MCP 配置与 Agent Skill 的确定性供应链安全扫描器。")
...
const scanCommand = program
    ...
    .option("-f, --format <format>", "报告格式：terminal、json、html 或 sarif", outputFormat, "terminal")
    .option("-o, --output <file>", "将报告写入文件")
    .option("--fail-on <severity>", "达到此严重级别时以 1 退出，never 表示永不拦截", failThreshold, "high")
    .option("--max-file-size <bytes>", "跳过大于此字节数的候选文件", positiveInteger, 1024 * 1024)
...
configureChineseHelp(scanCommand)
    ...
    try {
        ...
        if (options.output !== undefined) {
            ...
            if (!options.quiet) {
                process.stdout.write(`Aegis 已将 ${options.format} 报告写入 ${outputPath}\n`);
            }
        }
        else {
