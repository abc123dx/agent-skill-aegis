...
export function renderTerminal(
  ...
  const output: string[] = [
    ...
    paint(
      `  ${result.filesScanned} files · ${result.durationMs} ms · risk ${result.summary.riskScore}/100`,
      ansi.dim,
      color
    ),
    "",
  ...
  if (result.findings.length === 0) {
    output.push(
      paint("  ✓ No deterministic security findings.", ansi.green, color),
      ""
    );
    return output.join("\n");
  }
  ...
  for (const finding of result.findings) {
    ...
    output.push(
      ...
      `           ${finding.message}`,
      `           ${paint(`Fix: ${finding.recommendation}`, ansi.dim, color)}`,
      ""
    );
  }
...
<main>
  ...
  <section class="stats">
    ${(["critical", "high", "medium", "low", "info"] as const)
      .map(
        (severity) =>
          `<div class="stat ${severity}"><span>${severity.toUpperCase()}</span><b>${result.summary[severity]}</b></div>`
      )
      .join("")}
  </section>
  <nav class="filters" aria-label="Finding filters">
    ...
    ${(["critical", "high", "medium", "low", "info"] as const)
      .map(
        (severity) =>
          `<button data-filter="${severity}">${severity} ${result.summary[severity]}</button>`
      )
      .join("")}
  </nav>
